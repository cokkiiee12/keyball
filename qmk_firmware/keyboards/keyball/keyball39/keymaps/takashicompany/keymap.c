...
int16_t scroll_v_output_counter;   // 垂直スクロール出力を間引くためのカウンター
int16_t scroll_h_output_counter;   // 水平スクロール出力を間引くためのカウンター

int16_t scroll_v_threshold = TAKASHI_SCROLL_V_THRESHOLD;    // この閾値を超える度に垂直スクロールが実行される。 Vertical scrolling is performed each time this threshold is exceeded.
int16_t scroll_h_threshold = TAKASHI_SCROLL_H_THRESHOLD;    // この閾値を超える度に水平スクロールが実行される。 Each time this threshold is exceeded, horizontal scrolling is performed.
...
combo_t key_combos[] = {
    [COMBO_JK_BTN1]      = COMBO(jk_combo,  KC_MY_BTN1),
    [COMBO_KL_BTN2]      = COMBO(kl_combo,  KC_MY_BTN2),
    [COMBO_JL_SCR]       = COMBO(jl_combo,  KC_MY_SCR),
    [COMBO_JKL_GESTURE]  = COMBO(jkl_combo, KC_MY_GESTURE),
};

bool combo_should_trigger(uint16_t combo_index, combo_t *combo, uint16_t keycode, keyrecord_t *record) {
    return is_clickable_mode();
}
...
void disable_click_layer(void) {
    ...
    scroll_v_output_counter = 0;
    scroll_h_output_counter = 0;
}
    ...
    switch (keycode) {
        ...
        case KC_MY_GESTURE:
            if (record->event.pressed) {
                ...
                // 離した瞬間に方向を判定してアクションを発動
                if (my_abs(gesture_x) > my_abs(gesture_y)) {
                    if (gesture_x > GESTURE_THRESHOLD) {
                        tap_code16(LCTL(KC_RGHT));   // 右デスクトップへ
                    } else if (gesture_x < -GESTURE_THRESHOLD) {
                        tap_code16(LCTL(KC_LEFT));   // 左デスクトップへ
                    }
                } else {
                    if (gesture_y < -GESTURE_THRESHOLD) {
                        tap_code16(LCTL(KC_UP));     // Mission Control
                    } else if (gesture_y > GESTURE_THRESHOLD) {
                        tap_code16(KC_ESCAPE);       // Mission Control を閉じる
                    }
                }
                disable_click_layer();
            }
        ...
        case KC_EISU_KANA_TOG:
            if (record->event.pressed) {
                tap_code16(ime_is_kana ? KC_LNG2 : KC_LNG1);
                ime_is_kana = !ime_is_kana;
            }
            return false;
    ...
    if (current_x != 0 || current_y != 0) {
        ...
        switch (state) {
            ...
            case SCROLLING:
            {
                ...
                scroll_v_output_counter += rep_v;
                scroll_h_output_counter += rep_h;

                if (my_abs(scroll_v_output_counter) >= scroll_v_threshold * TAKASHI_SCROLL_OUTPUT_DIVISOR) {
                    current_v = -scroll_v_output_counter / scroll_v_threshold / TAKASHI_SCROLL_OUTPUT_DIVISOR * (user_config.mouse_scroll_v_reverse ? -1 : 1);
                    scroll_v_output_counter = 0;
                }
                if (my_abs(scroll_h_output_counter) >= scroll_h_threshold * TAKASHI_SCROLL_OUTPUT_DIVISOR) {
                    current_h = scroll_h_output_counter / scroll_h_threshold / TAKASHI_SCROLL_OUTPUT_DIVISOR * (user_config.mouse_scroll_h_reverse ? -1 : 1);
                    scroll_h_output_counter = 0;
                }
                current_x = 0;
                current_y = 0;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  LAYOUT_universal(
    _______  , _______  , _______  , _______  , _______  ,                            _______  , _______  , _______  , _______  , _______  ,
    _______  , KC_MY_BTN2, KC_MY_SCR, KC_MY_BTN1, KC_TRNS,                            KC_TRNS, _______  , _______  , _______  , _______  ,
    _______  , _______  , _______  , _______  , _______  ,                            _______  , _______  , _______  , _______  , _______  ,
    _______  , _______  , _______  , _______  , _______  , _______  ,      _______ ,  _______  , _______  , _______  , _______  , _______  
  )
};
